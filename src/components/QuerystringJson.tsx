...
import { qs2json, json2qs } from '../utils'
...
export default function QuerystringJson() {
  ...
  function transform() {
    ...
    if (type === 'json') {
      setResult(qs2json(code, charset))
    } else {
      setResult(json2qs(code, charset))
    }
  }
    ...
    <div>
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 1, sm: 1 }} style={{ margin: '2px 0' }}>
        <Select
          ...
          `}
        >
          <MenuItem value={'json'}>查询字符串转JSON</MenuItem>
          <MenuItem value={'qs'}>JSON转查询字符串</MenuItem>
        </Select>
        <Select
          labelId='select-option-label'
          id='select'
